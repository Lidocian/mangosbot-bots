...
#include "AssassinationDpsRogueStrategy.h"

using namespace ai;

class AssassinationDpsRogueStrategyActionNodeFactory : public NamedObjectFactory<ActionNode>
{
public:
   AssassinationDpsRogueStrategyActionNodeFactory()
    {
...
AssassinationDpsRogueStrategy::AssassinationDpsRogueStrategy(PlayerbotAI* ai) : MeleeCombatStrategy(ai)
{
    actionNodeFactories.Add(new AssassinationDpsRogueStrategyActionNodeFactory());
}

NextAction** AssassinationDpsRogueStrategy::getDefaultActions()
{
    return NextAction::array(0, new NextAction("melee", ACTION_NORMAL), NULL);
}

void AssassinationDpsRogueStrategy::InitTriggers(std::list<TriggerNode*> &triggers)
{
