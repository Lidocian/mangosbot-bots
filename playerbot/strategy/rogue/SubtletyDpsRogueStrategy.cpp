...
#include "SubtletyDpsRogueStrategy.h"

using namespace ai;

class SubtletyDpsRogueStrategyActionNodeFactory : public NamedObjectFactory<ActionNode>
{
public:
   SubtletyDpsRogueStrategyActionNodeFactory()
   {
...
SubtletyDpsRogueStrategy::SubtletyDpsRogueStrategy(PlayerbotAI* ai) : MeleeCombatStrategy(ai)
{
   actionNodeFactories.Add(new SubtletyDpsRogueStrategyActionNodeFactory());
}

NextAction** SubtletyDpsRogueStrategy::getDefaultActions()
{
   return NextAction::array(0, new NextAction("hemorrhage", ACTION_NORMAL), NULL);
}

void SubtletyDpsRogueStrategy::InitTriggers(std::list<TriggerNode*> &triggers)
{
